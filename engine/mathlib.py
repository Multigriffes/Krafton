...
class QUATERNION:
    ...
    def inverse(self):
        return QUATERNION(w=self.w / self.getLengthNoSqrt(), x=-self.x / self.getLengthNoSqrt(),
                          y=-self.y / self.getLengthNoSqrt(), z=-self.z / self.getLengthNoSqrt())
...
def crossProduct(a: QUATERNION,
                 b: QUATERNION) -> QUATERNION:  # Le cross product de deux vecteur donne un vecteur perpendiculaire aux deux autres, c'est l'équivalent d'une multiplication
...
def crossProductNormalized(a: QUATERNION,
                           b: QUATERNION) -> QUATERNION:  # https://en.wikipedia.org/wiki/Gram%E2%80%93Schmidt_process#/media/File:Gram-Schmidt_orthonormalization_process.gif
