...
class MAIN:
    ...
    def main(self):
        while self.run:
            ...
            gluLookAt(
                ...
                self.camera.front_vec.x + self.camera.coordinates[0],
                self.camera.front_vec.y + self.camera.coordinates[1],
                self.camera.front_vec.z + self.camera.coordinates[2],
    ...
    def checkForCollision(self):
        for block in ("Block_1", "Block_2", "Block_3", "Block_4"):
            ...
            for controller in ("left_controller", "right_controller"):
                ...
                if (selected_controller.collide_box[0][0] + selected_controller.coordinates[0] <=
                    selected_block.collide_box[1][0] + selected_block.coordinates[0] and
                    selected_controller.collide_box[1][0] + selected_controller.coordinates[0] >=
                    selected_block.collide_box[0][0] + selected_block.coordinates[0]) and (
                        selected_controller.collide_box[0][1] + selected_controller.coordinates[1] <=
                        selected_block.collide_box[1][1] + selected_block.coordinates[1] and
                        selected_controller.collide_box[1][1] + selected_controller.coordinates[1] >=
                        selected_block.collide_box[0][1] + selected_block.coordinates[1]) and (
                        selected_controller.collide_box[0][2] + selected_controller.coordinates[2] <=
                        selected_block.collide_box[1][2] + selected_block.coordinates[2] and
                        selected_controller.collide_box[1][2] + selected_controller.coordinates[2] >=
                        selected_block.collide_box[0][2] + selected_block.coordinates[2]):
    ...
    def parseAndCreateObjects(self):
        for object_to_be_created in project.objects:
            ...
            match selected_object['type']:
                case 'Faces':
                    my_object = FACES(vertices=object_file.vertices, quads=object_file.quads,
                                      triangles=object_file.triangles, normals=object_file.normals,
                                      coordinates=selected_object['coordinates'], color=selected_object['color'],
                                      collide_box=selected_object['collide_box'])
    ...
    def createDebugAxes(self):
        ...
        if project.debug_rotation_axes:  # Création des axes de rotation à partir des sin et cos
            rotation_axe_x = ROTATION_AXES(vertices=[(0.0, cos(radians(i)), sin(radians(i))) for i in range(0, 360, 1)],
                                           color=[1, 0, 0])
            rotation_axe_y = ROTATION_AXES(vertices=[(cos(radians(i)), 0.0, sin(radians(i))) for i in range(0, 360, 1)],
                                           color=[0, 1, 0])
            rotation_axe_z = ROTATION_AXES(vertices=[(cos(radians(i)), sin(radians(i)), 0.0) for i in range(0, 360, 1)],
                                           color=[0, 0, 1])
