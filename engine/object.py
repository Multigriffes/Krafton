...
class OBJECT_BASE:
    def __init__(self, collide_box: list = None, vertices: list = None, normals: list = None, triangles: list = None,
                 quads: list = None, coordinates: list = None, rotation: list = None, color: list = None) -> None:
    ...
    def draw(self, coordinates: list = None, rotation: list = None) -> None:
        if self.gl_list_id is None:
            ...
            glPushMatrix()
            glTranslatef(self.coordinates[0], self.coordinates[1],
                         self.coordinates[2]) if coordinates is None else glTranslatef(coordinates[0], coordinates[1],
                                                                                       coordinates[2])
...
class CAMERA:
    ...
    def addYaw(self, angle: float) -> None:
        ...
        quaternionForRotation = QUATERNION(w=usefulCos, x=usefulSin * self.up_vec.x, y=usefulSin * self.up_vec.y,
                                           z=usefulSin * self.up_vec.z)
    ...
    def addPitch(self, angle: float) -> None:
        ...
        usefulSin = sin(radiansHalfAngle)
        usefulCos = cos(radiansHalfAngle)
        quaternionForRotation = QUATERNION(w=usefulCos, x=usefulSin * self.right_vec.x, y=usefulSin * self.right_vec.y,
                                           z=usefulSin * self.right_vec.z)
        invertedQuaternionForRotation = quaternionForRotation.inverse()
    ...
    def addRoll(self, angle: float) -> None:
        ...
        usefulSin = sin(radiansHalfAngle)
        usefulCos = cos(radiansHalfAngle)
        quaternionForRotation = QUATERNION(w=usefulCos, x=usefulSin * self.front_vec.x, y=usefulSin * self.front_vec.y,
                                           z=usefulSin * self.front_vec.z)
        invertedQuaternionForRotation = quaternionForRotation.inverse()
    ...
    def compile(self) -> None:
        if self.gl_list_id is None:
            ...
            for i in range(len(self.quads[0])):
                ...
                glColor3fv((randint(0, 255) / 255, randint(0, 255) / 255,
                            randint(0, 255) / 255)) if self.color is None else glColor3fv(self.color)
